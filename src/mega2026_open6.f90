...
#elif PRM2MPI
  integer, parameter::mpi_proc_r=1,mpi_proc_z=2,mpi_proc_phi=1
#elif PRM1MPI
...
!2026-06-12s
  integer::iar_flux_grid(n_flux_size),jaz_flux_grid(n_flux_size)
  real(8)::aaa1_flux_grid(n_flux_size),aaa2_flux_grid(n_flux_size)
  real(8)::aaa3_flux_grid(n_flux_size),aaa4_flux_grid(n_flux_size)
!2026-06-12e
...
! include 'erfc.f90'
! #endif
include 'optimized_mod26JUNv4.f90' !2026-06-09

!--------------------------------------------------------------------
program mega2026
! MHD + Energetic Alpha Particles Simulation Code

! a bug corrected on 2026-06-15, suggested by Hanzheng Li
!   in subroutines divergence4, divergence3hm, divergence4hm for vr, vz, vphi, prs at (i,1,k), 
  
! modified on 2026-06-12
!   subroutines partsm1wopf, analyz_s, analyz_v, flux_coordinates, flux_c  are updated for AMDGPU

! modified on 2026-06-09
!   use optimized_mod26JUNv4.f90 (density(_gyro), moment(_gyro), emf_gyro are optimized fo MI300A)
!   random number generator for AMDGPU is commented out in subroutine ransuu

! modified on 2026-02-22
!   use optimized_mod26FEB.f90 (density(_gyro), moment(_gyro) are optimized fo MI300A)
!   subroutine com_particle3 is optimized for MI300A
...
!        call wall_clock(t2)
!        if(my_rank.eq.0)then
!          write(7,'(a,1pe14.5)')'harmonics=',t2-t1
!        end if        
...
!2026-06-12s
        iar_flux_grid(nb) = ia
        jaz_flux_grid(nb) = ja
        aaa1_flux_grid(nb) = aaa1
        aaa2_flux_grid(nb) = aaa2
        aaa3_flux_grid(nb) = aaa3
        aaa4_flux_grid(nb) = aaa4
!2026-06-12e
      end do
...
!2026-02-24s
!      call gradient(0,babs,gradbr,gradbz,gradbphi)
!      call rotation(0,hmr,hmz,hmphi,curvbr,curvbz,curvbphi)
!      call periodic_field_mlt6(gradbr,gradbz,gradbphi,curvbr,curvbz,curvbphi)
!2026-02-24e
...
#ifdef KTI
      if(ispecies.eq.0.or.ispecies.eq.1)then
        dv = (vmax-vmin)/dble(marker_num0) !2022-01-22 uniform loading in velocity
        do n = 1, marker_num0
          v(n) =  vmin + dv*(dble(n) - 0.50d0) !2022-01-22
        end do
      else
...
! distribution function
      ...
      if(ispecies.eq.0.or.ispecies.eq.1)then
        ...
        gc(10,n) = gc(1,n)*(temp_local/temp)**1.50d0*v(n)**2
...
!2026-02-24s
!      call gradient(0,pperp_i,c_ir,c_iz,c_iphi)
!      call gradient(0,pperp_a,c_ar,c_az,c_aphi)
!      call periodic_field_mlt6(c_ir,c_iz,c_iphi,c_ar,c_az,c_aphi)
!2026-02-24e
...
!$omp& private(l,n,m,ka,nb,ia,ja,value,i1)
      do l = 1, lpsi
      do n = -ntor, ntor
      do m = 0, mpol
      ...
      do nb = 1, n_flux_grid
        if(l_flux_grid(nb) /= l) cycle

! interpolated value
        ia = iar_flux_grid(nb)
        ja = jaz_flux_grid(nb)
        value = scal(ia, ja,  ka )*aaa1_flux_grid(nb)  + scal(ia+1,ja,  ka )*aaa2_flux_grid(nb) &
              + scal(ia, ja+1,ka )*aaa3_flux_grid(nb)  + scal(ia+1,ja+1,ka )*aaa4_flux_grid(nb)
...
#ifdef AMDGPU      
!$omp target teams distribute parallel do collapse(3) &
!$omp& private(l,n,m,ka,nb,ia,ja,value_r,value_z,value_phi)
!$omp& private(val_r,val_t,val_p,i1)
      do l = 1, lpsi
      do n = -ntor, ntor
      do m = 0, mpol
...
! interpolated value
        ...
        value_r = wr(ia, ja,  ka )*aaa1_flux_grid(nb)  + wr(ia+1,ja,  ka )*aaa2_flux_grid(nb) &
                + wr(ia, ja+1,ka )*aaa3_flux_grid(nb)  + wr(ia+1,ja+1,ka )*aaa4_flux_grid(nb)
        value_z = wz(ia, ja,  ka )*aaa1_flux_grid(nb)  + wz(ia+1,ja,  ka )*aaa2_flux_grid(nb) &
                + wz(ia, ja+1,ka )*aaa3_flux_grid(nb)  + wz(ia+1,ja+1,ka )*aaa4_flux_grid(nb)
        value_phi = wphi(ia, ja,  ka )*aaa1_flux_grid(nb)  + wphi(ia+1,ja,  ka )*aaa2_flux_grid(nb) &
                  + wphi(ia, ja+1,ka )*aaa3_flux_grid(nb)  + wphi(ia+1,ja+1,ka )*aaa4_flux_grid(nb)
...
!--------------------------------------------------------------------
subroutine periodic_field_mlt4(xxx1,xxx2,xxx3,xxx4)
! boundary condition with mpi communications for 4 variables
! 2025-10-03
!--------------------------------------------------------------------
      use parameters
      use mpiset
...
!--------------------------------------------------------------------
subroutine periodic_field_mlt6(xxx1,xxx2,xxx3,xxx4,xxx5,xxx6)
! boundary condition with mpi communications for 6 variables
! 2026-02-22
!--------------------------------------------------------------------
      use parameters
      use mpiset
      ...
      real(8)::xxx1(lr,lz,lphi),xxx2(lr,lz,lphi),xxx3(lr,lz,lphi),xxx4(lr,lz,lphi)
      real(8)::xxx5(lr,lz,lphi),xxx6(lr,lz,lphi)
      integer::i,j,k
      integer::node_up,node_down,lrz2,lrphi2,lzphi2
...
! !$omp target update to(nup,ndown,nup_r,ndown_r)

#ifdef AMDGPU
!$omp target teams distribute parallel do private(igc, idx)
!$omp& map(to: node, node_now) map(tofrom: gc, nup, ndown, send_up, send_down, nsend_up, nsend_down)
      do n = 1, marker_num
...
#ifdef AMDGPU
!$omp target teams distribute parallel do private(i,n,igc) thread_limit(256)
#else      
!$omp parallel do private(n)
#endif
      do i = 1, nloop
...
!--------------------------------------------------------------------
subroutine partsm1wopf(xxx,cww)
! subroutine periodic_field is not called from this subroutine
! 2026-06-11, for stable operation on MI300A:
!    array aaa is changed to allocatable, save
!    !$omp target data map is used
!--------------------------------------------------------------------
      use grid
      implicit none
...
!      real(8)::xxx(lr,lz,lphi),aaa(lr,lz,lphi)
      ...
      integer::i,j,k,i1,i2,j1,j2,k1,k2
!2026-06-11s
      real(8), allocatable, save::aaa(:,:,:)
      logical, save::started=.false.

      if (.not. started) then
        allocate(aaa(lr,lz,lphi))
        started = .true.
      end if
!2026-06-11e
...
#ifdef AMDGPU
! !$omp target data map(tofrom: xxx) map(alloc: aaa) !2026-06-11
...
! #ifdef AMDGPU
! !$omp end target data !2026-06-11
! #endif

! subroutines periodic_field_mltX are called outside of this subroutine, 2026-02-22
! periodic condition
!      call periodic_field(xxx)
...
!$omp parallel do
      ...
      do k = 3, lphi-2
      do i = 1, lr
! correction for vr, vz, vphi, prs at (i,1,k), suggested by Hanzheng Li, 2026-06-15
        dvr(i,1,   k) = dvr(i,1,   k) &
              + (vr(i,3,k)*uz(i,3,k)*cc1 + vr(i,2,k)*uz(i,2,k)*cc0 - vr(i,1,k)*uz(i,1,k)*cc2 )*dz1
        dvz(i,1,   k) = dvz(i,1,   k) &
              + (vz(i,3,k)*uz(i,3,k)*cc1 + vz(i,2,k)*uz(i,2,k)*cc0 - vz(i,1,k)*uz(i,1,k)*cc2 )*dz1 &
              + (prs(i,3,k)*cc1 + prs(i,2,k)*cc0 - prs(i,1,1)*cc2 )*dz1
        dvphi(i,1,   k) = dvphi(i,1,   k) &
              + (vphi(i,3,k)*uz(i,3,k)*cc1 + vphi(i,2,k)*uz(i,2,k)*cc0 - vphi(i,1,k)*uz(i,1,k)*cc2 )*dz1
        dprs(i,1,   k) = dprs(i,1,   k) &
              + (prs(i,3,k)*uz(i,3,k)*cc1 + prs(i,2,k)*uz(i,2,k)*cc0 - prs(i,1,k)*uz(i,1,k)*cc2 )*dz1
...
!$omp parallel do
      ...
      do k = 3, lphi-2
      do i = 1, lr
! correction for vr, vz, vphi at (i,1,k), suggested by Hanzheng Li, 2026-06-15
        dvr(i,1,   k) = dvr(i,1,   k) &
              + (vr(i,3,k)*vdrftz(i,3,k)*cc1 + vr(i,2,k)*vdrftz(i,2,k)*cc0 &
               - vr(i,1,k)*vdrftz(i,1,k)*cc2 )*dz1
        dvz(i,1,   k) = dvz(i,1,   k) &
              + (vz(i,3,k)*vdrftz(i,3,k)*cc1 + vz(i,2,k)*vdrftz(i,2,k)*cc0 &
               - vz(i,1,k)*vdrftz(i,1,k)*cc2 )*dz1 &
              + (prs(i,3,k)*cc1 + prs(i,2,k)*cc0 - prs(i,1,1)*cc2 )*dz1
        dvphi(i,1,   k) = dvphi(i,1,   k) &
              + (vphi(i,3,k)*vdrftz(i,3,k)*cc1 + vphi(i,2,k)*vdrftz(i,2,k)*cc0 &
               - vphi(i,1,k)*vdrftz(i,1,k)*cc2 )*dz1
