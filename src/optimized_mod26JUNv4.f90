!---------------------------------------------------------------------------------
! modules: aurora, fx100, fugaku, xeon6900p, amdgpu (MI300A)
...
! 2026-02-22: module amdgpu: density(_gyro) and moment(_gyro) are furhter optimized
! 2026-06-06: module amdgpu: density(_gyro), moment(_gyro), emf_gyro are furhter optimized
!---------------------------------------------------------------------------------
...
!2024-12-21s, correction suggested by Panith Adulsiriswad
      ...
      call partsm1wopf(dns,cwwa)
      call partsm1wopf(mom,cwwa)

      call partsm1wopf(ppara,cwwa)
      call partsm1wopf(pperp,cwwa)
!2024-12-21e
! 2026-02-22, partsm1wopf does not impose periodic condition
       call periodic_field_mlt4(dns,mom,ppara,pperp)
...
!2024-12-21s, correction suggested by Panith Adulsiriswad
       ...
       call partsm1wopf(qpara,cwwa)
       call partsm1wopf(qperp,cwwa)
!2024-12-21e
! 2026-02-22, partsm1wopf does not impose periodic condition
       call periodic_field_mlt6(dns,mom,ppara,pperp,qpara,qperp)
...
!2026-02-22s
      ...
      real(8)::aaa(8,marker_each)
!2026-02-22e
      real(8)::work(8,4,0:lr,0:lz,0:lphi) !2026-06-06
...
!2026-06-06s
      ...
      integer::m,idx,idx_dead
      real(8)::s1,s2,s3,s4,s5,s6,s7,s8,u1,u2,u3,u4,u5,u6,u7,u8
      real(8)::v1,v2,v3,v4,v5,v6,v7,v8,w1,w2,w3,w4,w5,w6,w7,w8
      real(8)::t0,t1,t2
!2026-06-06e
!      real(8)::d0,d1,d2,d3,d4,d5,t1,t2,t3,t4,t5
...
!$omp target teams distribute parallel do private(i)
      do i = 1, lrzphi
        ...
        ppara(i,1,1) = 0.0d0
        pperp(i,1,1) = 0.0d0
        imrk_grid(i,1,1) = 0 !2026-06-06
      end do
      imrk_dead = 0 !2026-06-06
...
!$omp target teams distribute parallel do private(i,j,k) collapse(3)
      ...
      do j = 0, lz
      do i = 0, lr
        work(1,1,i,j,k) = 0.0d0
        work(2,1,i,j,k) = 0.0d0
        work(3,1,i,j,k) = 0.0d0
        work(4,1,i,j,k) = 0.0d0
        work(5,1,i,j,k) = 0.0d0
        work(6,1,i,j,k) = 0.0d0
        work(7,1,i,j,k) = 0.0d0
        work(8,1,i,j,k) = 0.0d0

        work(1,2,i,j,k) = 0.0d0
        work(2,2,i,j,k) = 0.0d0
        work(3,2,i,j,k) = 0.0d0
        work(4,2,i,j,k) = 0.0d0
        work(5,2,i,j,k) = 0.0d0
        work(6,2,i,j,k) = 0.0d0
        work(7,2,i,j,k) = 0.0d0
        work(8,2,i,j,k) = 0.0d0

        work(1,3,i,j,k) = 0.0d0
        work(2,3,i,j,k) = 0.0d0
        work(3,3,i,j,k) = 0.0d0
        work(4,3,i,j,k) = 0.0d0
        work(5,3,i,j,k) = 0.0d0
        work(6,3,i,j,k) = 0.0d0
        work(7,3,i,j,k) = 0.0d0
        work(8,3,i,j,k) = 0.0d0

        work(1,4,i,j,k) = 0.0d0
        work(2,4,i,j,k) = 0.0d0
        work(3,4,i,j,k) = 0.0d0
        work(4,4,i,j,k) = 0.0d0
        work(5,4,i,j,k) = 0.0d0
        work(6,4,i,j,k) = 0.0d0
        work(7,4,i,j,k) = 0.0d0
        work(8,4,i,j,k) = 0.0d0
      end do
      end do
      end do
!2026-06-06e
...
!$omp target teams distribute parallel do 
!$omp& private(n,ar1,ar,az1,az,aphi1,aphi)
      do n = 1, marker_num
        ijk_a(1,n)=max(1,min(lr  -1,int((gc(1,n)-ma_mi_r)*dr1  ) + kr  ))
        ijk_a(2,n)=max(1,min(lz  -1,int(gc(2,n)          *dz1  ) + kz  ))
        ijk_a(3,n)=max(1,min(lphi-1,int(gc(3,n)        *dphi1) + kphi))
...
!$omp  target teams distribute parallel do
!$omp& private(n,ia,ja,ka,idx,idx_dead)
      do n = 1, marker_num
        ia=ijk_a(1,n)
        ja=ijk_a(2,n)
        ka=ijk_a(3,n)
!$omp atomic capture
        ...
        idx = imrk_grid(ia,ja,ka)
!$omp end atomic

        if(idx.le.marker_local_max)then
          marker_index(idx,ia,ja,ka) = n
        else
!$omp atomic write
        imrk_grid(ia,ja,ka) = marker_local_max
...
!$omp end atomic
        ...
        marker_index_dead(idx_dead) = n
        end if
      end do

!$omp  target teams distribute parallel do collapse(3) thread_limit(256) &
!$omp& private(n,ia,ja,ka,m,p0,p1,p2,mu1)
!$omp& private(s1,s2,s3,s4,s5,s6,s7,s8,u1,u2,u3,u4,u5,u6,u7,u8)
!$omp& private(v1,v2,v3,v4,v5,v6,v7,v8,w1,w2,w3,w4,w5,w6,w7,w8)
      ...
      do ia = 1, lr-1
        ...
        u1 = 0.0d0; u2 = 0.0d0; u3 = 0.0d0; u4 = 0.0d0
        u5 = 0.0d0; u6 = 0.0d0; u7 = 0.0d0; u8 = 0.0d0
        v1 = 0.0d0; v2 = 0.0d0; v3 = 0.0d0; v4 = 0.0d0
        v5 = 0.0d0; v6 = 0.0d0; v7 = 0.0d0; v8 = 0.0d0
        w1 = 0.0d0; w2 = 0.0d0; w3 = 0.0d0; w4 = 0.0d0
        w5 = 0.0d0; w6 = 0.0d0; w7 = 0.0d0; w8 = 0.0d0
...
! gc_x(:,n) = 1:r, 2:z, 3:phi, 4:p, 5:mu, 6:weight, 7:active, 8:p_phi, 9:fff, 10:fnrml
        ...
        work(1, 1, ia, ja, ka ) = work(1, 1, ia, ja, ka ) + s1
        work(2, 1, ia, ja, ka ) = work(2, 1, ia, ja, ka ) + s2
        work(3, 1, ia, ja, ka ) = work(3, 1, ia, ja, ka ) + s3
        work(4, 1, ia, ja, ka ) = work(4, 1, ia, ja, ka ) + s4
        work(5, 1, ia, ja, ka ) = work(5, 1, ia, ja, ka ) + s5
        work(6, 1, ia, ja, ka ) = work(6, 1, ia, ja, ka ) + s6
        work(7, 1, ia, ja, ka ) = work(7, 1, ia, ja, ka ) + s7
        work(8, 1, ia, ja, ka ) = work(8, 1, ia, ja, ka ) + s8

        work(1, 2, ia, ja, ka ) = work(1, 2, ia, ja, ka ) + u1
        work(2, 2, ia, ja, ka ) = work(2, 2, ia, ja, ka ) + u2
        work(3, 2, ia, ja, ka ) = work(3, 2, ia, ja, ka ) + u3
        work(4, 2, ia, ja, ka ) = work(4, 2, ia, ja, ka ) + u4
        work(5, 2, ia, ja, ka ) = work(5, 2, ia, ja, ka ) + u5
        work(6, 2, ia, ja, ka ) = work(6, 2, ia, ja, ka ) + u6
        work(7, 2, ia, ja, ka ) = work(7, 2, ia, ja, ka ) + u7
        work(8, 2, ia, ja, ka ) = work(8, 2, ia, ja, ka ) + u8

        work(1, 3, ia, ja, ka ) = work(1, 3, ia, ja, ka ) + v1
        work(2, 3, ia, ja, ka ) = work(2, 3, ia, ja, ka ) + v2
        work(3, 3, ia, ja, ka ) = work(3, 3, ia, ja, ka ) + v3
        work(4, 3, ia, ja, ka ) = work(4, 3, ia, ja, ka ) + v4
        work(5, 3, ia, ja, ka ) = work(5, 3, ia, ja, ka ) + v5
        work(6, 3, ia, ja, ka ) = work(6, 3, ia, ja, ka ) + v6
        work(7, 3, ia, ja, ka ) = work(7, 3, ia, ja, ka ) + v7
        work(8, 3, ia, ja, ka ) = work(8, 3, ia, ja, ka ) + v8

        work(1, 4, ia, ja, ka ) = work(1, 4, ia, ja, ka ) + w1
        work(2, 4, ia, ja, ka ) = work(2, 4, ia, ja, ka ) + w2
        work(3, 4, ia, ja, ka ) = work(3, 4, ia, ja, ka ) + w3
        work(4, 4, ia, ja, ka ) = work(4, 4, ia, ja, ka ) + w4
        work(5, 4, ia, ja, ka ) = work(5, 4, ia, ja, ka ) + w5
        work(6, 4, ia, ja, ka ) = work(6, 4, ia, ja, ka ) + w6
        work(7, 4, ia, ja, ka ) = work(7, 4, ia, ja, ka ) + w7
        work(8, 4, ia, ja, ka ) = work(8, 4, ia, ja, ka ) + w8
      end do
      end do
      end do

!$omp  target teams distribute parallel do
!$omp& private(m,n,ia,ja,ka)
!$omp& private(p0,p1,p2,mu1)
       do m = 1, imrk_dead
...
!$omp atomic update
        work(1, 1, ia, ja, ka ) = work(1, 1, ia, ja, ka ) + aaa(1,n)*p0
!$omp atomic update
        work(2, 1, ia, ja, ka ) = work(2, 1, ia, ja, ka ) + aaa(2,n)*p0
!$omp atomic update
        work(3, 1, ia, ja, ka ) = work(3, 1, ia, ja, ka ) + aaa(3,n)*p0
!$omp atomic update
        work(4, 1, ia, ja, ka ) = work(4, 1, ia, ja, ka ) + aaa(4,n)*p0
!$omp atomic update
        work(5, 1, ia, ja, ka ) = work(5, 1, ia, ja, ka ) + aaa(5,n)*p0
!$omp atomic update
        work(6, 1, ia, ja, ka ) = work(6, 1, ia, ja, ka ) + aaa(6,n)*p0
!$omp atomic update
        work(7, 1, ia, ja, ka ) = work(7, 1, ia, ja, ka ) + aaa(7,n)*p0
!$omp atomic update
        work(8, 1, ia, ja, ka ) = work(8, 1, ia, ja, ka ) + aaa(8,n)*p0

!$omp atomic update
        work(1, 2, ia, ja, ka ) = work(1, 2, ia, ja, ka ) + aaa(1,n)*p1
!$omp atomic update
        work(2, 2, ia, ja, ka ) = work(2, 2, ia, ja, ka ) + aaa(2,n)*p1
!$omp atomic update
        work(3, 2, ia, ja, ka ) = work(3, 2, ia, ja, ka ) + aaa(3,n)*p1
!$omp atomic update
        work(4, 2, ia, ja, ka ) = work(4, 2, ia, ja, ka ) + aaa(4,n)*p1
!$omp atomic update
        work(5, 2, ia, ja, ka ) = work(5, 2, ia, ja, ka ) + aaa(5,n)*p1
!$omp atomic update
        work(6, 2, ia, ja, ka ) = work(6, 2, ia, ja, ka ) + aaa(6,n)*p1
!$omp atomic update
        work(7, 2, ia, ja, ka ) = work(7, 2, ia, ja, ka ) + aaa(7,n)*p1
!$omp atomic update
        work(8, 2, ia, ja, ka ) = work(8, 2, ia, ja, ka ) + aaa(8,n)*p1

!$omp atomic update
        work(1, 3, ia, ja, ka ) = work(1, 3, ia, ja, ka ) + aaa(1,n)*p2
!$omp atomic update
        work(2, 3, ia, ja, ka ) = work(2, 3, ia, ja, ka ) + aaa(2,n)*p2
!$omp atomic update
        work(3, 3, ia, ja, ka ) = work(3, 3, ia, ja, ka ) + aaa(3,n)*p2
!$omp atomic update
        work(4, 3, ia, ja, ka ) = work(4, 3, ia, ja, ka ) + aaa(4,n)*p2
!$omp atomic update
        work(5, 3, ia, ja, ka ) = work(5, 3, ia, ja, ka ) + aaa(5,n)*p2
!$omp atomic update
        work(6, 3, ia, ja, ka ) = work(6, 3, ia, ja, ka ) + aaa(6,n)*p2
!$omp atomic update
        work(7, 3, ia, ja, ka ) = work(7, 3, ia, ja, ka ) + aaa(7,n)*p2
!$omp atomic update
        work(8, 3, ia, ja, ka ) = work(8, 3, ia, ja, ka ) + aaa(8,n)*p2

!$omp atomic update
        work(1, 4, ia, ja, ka ) = work(1, 4, ia, ja, ka ) + aaa(1,n)*mu1
!$omp atomic update
        work(2, 4, ia, ja, ka ) = work(2, 4, ia, ja, ka ) + aaa(2,n)*mu1
!$omp atomic update
        work(3, 4, ia, ja, ka ) = work(3, 4, ia, ja, ka ) + aaa(3,n)*mu1
!$omp atomic update
        work(4, 4, ia, ja, ka ) = work(4, 4, ia, ja, ka ) + aaa(4,n)*mu1
!$omp atomic update
        work(5, 4, ia, ja, ka ) = work(5, 4, ia, ja, ka ) + aaa(5,n)*mu1
!$omp atomic update
        work(6, 4, ia, ja, ka ) = work(6, 4, ia, ja, ka ) + aaa(6,n)*mu1
!$omp atomic update
        work(7, 4, ia, ja, ka ) = work(7, 4, ia, ja, ka ) + aaa(7,n)*mu1
!$omp atomic update
        work(8, 4, ia, ja, ka ) = work(8, 4, ia, ja, ka ) + aaa(8,n)*mu1
       end do

!$omp target teams distribute parallel do private(ia,ja,ka) collapse(3)
       do ka = 1, lphi
       do ja = 1, lz
       do ia = 1, lr
         dns(ia,  ja,  ka  ) = dns(ia,  ja,  ka  ) &
                             + work(8,1,ia-1,ja-1,ka-1) + work(7,1,ia,  ja-1,ka-1) &
                             + work(6,1,ia-1,ja,  ka-1) + work(5,1,ia,  ja,  ka-1) &
                             + work(4,1,ia-1,ja-1,ka  ) + work(3,1,ia,  ja-1,ka  ) &
                             + work(2,1,ia-1,ja,  ka  ) + work(1,1,ia,  ja,  ka  )
         mom(ia,  ja,  ka  ) = mom(ia,  ja,  ka  ) &
                             + work(8,2,ia-1,ja-1,ka-1) + work(7,2,ia,  ja-1,ka-1) &
                             + work(6,2,ia-1,ja,  ka-1) + work(5,2,ia,  ja,  ka-1) &
                             + work(4,2,ia-1,ja-1,ka  ) + work(3,2,ia,  ja-1,ka  ) &
                             + work(2,2,ia-1,ja,  ka  ) + work(1,2,ia,  ja,  ka  )
         ppara(ia,  ja,  ka) = ppara(ia,  ja,  ka) &
                             + work(8,3,ia-1,ja-1,ka-1) + work(7,3,ia,  ja-1,ka-1) &
                             + work(6,3,ia-1,ja,  ka-1) + work(5,3,ia,  ja,  ka-1) &
                             + work(4,3,ia-1,ja-1,ka  ) + work(3,3,ia,  ja-1,ka  ) &
                             + work(2,3,ia-1,ja,  ka  ) + work(1,3,ia,  ja,  ka  )
         pperp(ia,  ja,  ka) = pperp(ia,  ja,  ka) &
                             + work(8,4,ia-1,ja-1,ka-1) + work(7,4,ia,  ja-1,ka-1) &
                             + work(6,4,ia-1,ja,  ka-1) + work(5,4,ia,  ja,  ka-1) &
                             + work(4,4,ia-1,ja-1,ka  ) + work(3,4,ia,  ja-1,ka  ) &
                             + work(2,4,ia-1,ja,  ka  ) + work(1,4,ia,  ja,  ka  )
       end do
       end do
       end do
...
!2026-02-22s
      ...
      real(8)::aaa(8,marker_each)
!2026-02-22e
      real(8)::work(8,6,0:lr,0:lz,0:lphi) !2026-06-06
      integer::i,j,k,l,n,ia,ia1,ja,ja1,ka,ka1,lr1,lz1
      real(8)::ar,ar1,az,az1,aphi,aphi1
!      real(8)::aaa1,aaa2,aaa3,aaa4,aaa5,aaa6,aaa7,aaa8
      real(8)::vol,cwwa,cwwc
...
!2026-06-06s
      ...
      real(8)::s1,s2,s3,s4,s5,s6,s7,s8,u1,u2,u3,u4,u5,u6,u7,u8
      real(8)::v1,v2,v3,v4,v5,v6,v7,v8,w1,w2,w3,w4,w5,w6,w7,w8
      real(8)::x1,x2,x3,x4,x5,x6,x7,x8,y1,y2,y3,y4,y5,y6,y7,y8
!2026-06-06e
!      real(8)::d0,d1,d2,d3,d4,d5,t1,t2,t3,t4,t5
...
!$omp target teams distribute parallel do private(i,j,k) collapse(3)
      ...
      do i = 0, lr
        ...
        work(1,5,i,j,k) = 0.0d0
        work(2,5,i,j,k) = 0.0d0
        work(3,5,i,j,k) = 0.0d0
        work(4,5,i,j,k) = 0.0d0
        work(5,5,i,j,k) = 0.0d0
        work(6,5,i,j,k) = 0.0d0
        work(7,5,i,j,k) = 0.0d0
        work(8,5,i,j,k) = 0.0d0

        work(1,6,i,j,k) = 0.0d0
        work(2,6,i,j,k) = 0.0d0
        work(3,6,i,j,k) = 0.0d0
        work(4,6,i,j,k) = 0.0d0
        work(5,6,i,j,k) = 0.0d0
        work(6,6,i,j,k) = 0.0d0
        work(7,6,i,j,k) = 0.0d0
        work(8,6,i,j,k) = 0.0d0
      end do
      end do
      end do
!2026-06-06e
...
!$omp  target teams distribute parallel do collapse(3) thread_limit(256) &
!$omp& private(n,ia,ja,ka,m,p0,p1,p2,mu1,p3,p1mu)
!$omp& private(s1,s2,s3,s4,s5,s6,s7,s8,u1,u2,u3,u4,u5,u6,u7,u8)
!$omp& private(v1,v2,v3,v4,v5,v6,v7,v8,w1,w2,w3,w4,w5,w6,w7,w8)
!$omp& private(x1,x2,x3,x4,x5,x6,x7,x8,y1,y2,y3,y4,y5,y6,y7,y8)
      ...
      do ia = 1, lr-1
        ...
        w1 = 0.0d0; w2 = 0.0d0; w3 = 0.0d0; w4 = 0.0d0
        w5 = 0.0d0; w6 = 0.0d0; w7 = 0.0d0; w8 = 0.0d0
        x1 = 0.0d0; x2 = 0.0d0; x3 = 0.0d0; x4 = 0.0d0
        x5 = 0.0d0; x6 = 0.0d0; x7 = 0.0d0; x8 = 0.0d0
        y1 = 0.0d0; y2 = 0.0d0; y3 = 0.0d0; y4 = 0.0d0
        y5 = 0.0d0; y6 = 0.0d0; y7 = 0.0d0; y8 = 0.0d0
...
! gc_x(:,n) = 1:r, 2:z, 3:phi, 4:p, 5:mu, 6:weight, 7:active, 8:p_phi, 9:fff, 10:fnrml
        ...
        work(1, 5, ia, ja, ka ) = work(1, 5, ia, ja, ka ) + x1
        work(2, 5, ia, ja, ka ) = work(2, 5, ia, ja, ka ) + x2
        work(3, 5, ia, ja, ka ) = work(3, 5, ia, ja, ka ) + x3
        work(4, 5, ia, ja, ka ) = work(4, 5, ia, ja, ka ) + x4
        work(5, 5, ia, ja, ka ) = work(5, 5, ia, ja, ka ) + x5
        work(6, 5, ia, ja, ka ) = work(6, 5, ia, ja, ka ) + x6
        work(7, 5, ia, ja, ka ) = work(7, 5, ia, ja, ka ) + x7
        work(8, 5, ia, ja, ka ) = work(8, 5, ia, ja, ka ) + x8

        work(1, 6, ia, ja, ka ) = work(1, 6, ia, ja, ka ) + y1
        work(2, 6, ia, ja, ka ) = work(2, 6, ia, ja, ka ) + y2
        work(3, 6, ia, ja, ka ) = work(3, 6, ia, ja, ka ) + y3
        work(4, 6, ia, ja, ka ) = work(4, 6, ia, ja, ka ) + y4
        work(5, 6, ia, ja, ka ) = work(5, 6, ia, ja, ka ) + y5
        work(6, 6, ia, ja, ka ) = work(6, 6, ia, ja, ka ) + y6
        work(7, 6, ia, ja, ka ) = work(7, 6, ia, ja, ka ) + y7
        work(8, 6, ia, ja, ka ) = work(8, 6, ia, ja, ka ) + y8
      end do
      end do
      end do
...
!$omp  target teams distribute parallel do
!$omp& private(m,n,ia,ja,ka)
!$omp& private(p0,p1,p2,mu1,p3,p1mu)
       do m = 1, imrk_dead
        n =  marker_index_dead(m)
...
!$omp atomic update
        work(1, 5, ia, ja, ka ) = work(1, 5, ia, ja, ka ) + aaa(1,n)*p3
!$omp atomic update
        work(2, 5, ia, ja, ka ) = work(2, 5, ia, ja, ka ) + aaa(2,n)*p3
!$omp atomic update
        work(3, 5, ia, ja, ka ) = work(3, 5, ia, ja, ka ) + aaa(3,n)*p3
!$omp atomic update
        work(4, 5, ia, ja, ka ) = work(4, 5, ia, ja, ka ) + aaa(4,n)*p3
!$omp atomic update
        work(5, 5, ia, ja, ka ) = work(5, 5, ia, ja, ka ) + aaa(5,n)*p3
!$omp atomic update
        work(6, 5, ia, ja, ka ) = work(6, 5, ia, ja, ka ) + aaa(6,n)*p3
!$omp atomic update
        work(7, 5, ia, ja, ka ) = work(7, 5, ia, ja, ka ) + aaa(7,n)*p3
!$omp atomic update
        work(8, 5, ia, ja, ka ) = work(8, 5, ia, ja, ka ) + aaa(8,n)*p3

!$omp atomic update
        work(1, 6, ia, ja, ka ) = work(1, 6, ia, ja, ka ) + aaa(1,n)*p1mu
!$omp atomic update
        work(2, 6, ia, ja, ka ) = work(2, 6, ia, ja, ka ) + aaa(2,n)*p1mu
!$omp atomic update
        work(3, 6, ia, ja, ka ) = work(3, 6, ia, ja, ka ) + aaa(3,n)*p1mu
!$omp atomic update
        work(4, 6, ia, ja, ka ) = work(4, 6, ia, ja, ka ) + aaa(4,n)*p1mu
!$omp atomic update
        work(5, 6, ia, ja, ka ) = work(5, 6, ia, ja, ka ) + aaa(5,n)*p1mu
!$omp atomic update
        work(6, 6, ia, ja, ka ) = work(6, 6, ia, ja, ka ) + aaa(6,n)*p1mu
!$omp atomic update
        work(7, 6, ia, ja, ka ) = work(7, 6, ia, ja, ka ) + aaa(7,n)*p1mu
!$omp atomic update
        work(8, 6, ia, ja, ka ) = work(8, 6, ia, ja, ka ) + aaa(8,n)*p1mu
       end do
...
!$omp target teams distribute parallel do private(ia,ja,ka) collapse(3)
       ...
       do ia = 1, lr
         ...
         pperp(ia,  ja,  ka) = pperp(ia,  ja,  ka) &
                             ...
                             + work(4,4,ia-1,ja-1,ka  ) + work(3,4,ia,  ja-1,ka  ) &
                             + work(2,4,ia-1,ja,  ka  ) + work(1,4,ia,  ja,  ka  )
         qpara(ia,  ja,  ka) = qpara(ia,  ja,  ka) &
                             + work(8,5,ia-1,ja-1,ka-1) + work(7,5,ia,  ja-1,ka-1) &
                             + work(6,5,ia-1,ja,  ka-1) + work(5,5,ia,  ja,  ka-1) &
                             + work(4,5,ia-1,ja-1,ka  ) + work(3,5,ia,  ja-1,ka  ) &
                             + work(2,5,ia-1,ja,  ka  ) + work(1,5,ia,  ja,  ka  )
         qperp(ia,  ja,  ka) = qperp(ia,  ja,  ka) &
                             + work(8,6,ia-1,ja-1,ka-1) + work(7,6,ia,  ja-1,ka-1) &
                             + work(6,6,ia-1,ja,  ka-1) + work(5,6,ia,  ja,  ka-1) &
                             + work(4,6,ia-1,ja-1,ka  ) + work(3,6,ia,  ja-1,ka  ) &
                             + work(2,6,ia-1,ja,  ka  ) + work(1,6,ia,  ja,  ka  )
       end do
       end do
       end do
...
!      call wall_clock(t2)
!      if(my_rank.eq.0)then
!        write(7,'(a,1pe14.5)')'emf_gyro 0',t2-t1
!      endif
!      call wall_clock(t1)

!$omp target teams distribute parallel do &
!$omp& private(n,nc,in,ia,ja,ka,ar1,ar,az1,az,aphi1,aphi) &
!$omp& private(aaa1,aaa2,aaa3,aaa4,aaa5,aaa6,aaa7,aaa8)
      do n = 1, marker_num_gyro
        nc =(n-1)/ngyro + 1
...
!      call wall_clock(t2)
!      if(my_rank.eq.0)then
!        write(7,'(a,1pe14.5)')'emf_gyro 1',t2-t1
!      endif
!      call wall_clock(t1)

!$omp target teams distribute parallel do collapse(2) private(in,nc)
      do nc = 1, marker_num
        do in = 1, nflp_gyro
          flp(in,nc) =(flp_gyro(in,ngyro*(nc-1)+1) &
                      +flp_gyro(in,ngyro*(nc-1)+2) &
...
!      call wall_clock(t2)
!      if(my_rank.eq.0)then
!        write(7,'(a,1pe14.5)')'emf_gyro 2',t2-t1
!      endif
...
!2026-06-06s
      integer::marker_index(marker_local_max,lr,lz,lphi),imrk_grid(lr,lz,lphi)
      integer::marker_index_dead(marker_each_gyro),imrk_dead
      integer::m,idx,idx_dead
      real(8)::s1,s2,s3,s4,s5,s6,s7,s8,u1,u2,u3,u4,u5,u6,u7,u8
      real(8)::v1,v2,v3,v4,v5,v6,v7,v8,w1,w2,w3,w4,w5,w6,w7,w8
      real(8)::t0,t1,t2
...
!      call wall_clock(t2)
!      if(my_rank.eq.0)then
!        write(7,'(a,1pe14.5)')'density_gyro 0',t2-t1
!      endif
!      call wall_clock(t1)
        
!$omp target teams distribute parallel do 
!$omp& private(n,nc,ar1,ar,az1,az,aphi1,aphi)
      do n = 1, marker_num_gyro
        nc =(n-1)/ngyro + 1
        ijk_a(1,n)=max(1,min(lr  -1,int((gyro_phys(1,n)-ma_mi_r)*dr1  ) + kr  ))
        ijk_a(2,n)=max(1,min(lz  -1,int(gyro_phys(2,n)          *dz1  ) + kz  ))
...
!      call wall_clock(t2)
!      if(my_rank.eq.0)then
!        write(7,'(a,1pe14.5)')'density_gyro 1',t2-t1
!      endif
!      call wall_clock(t1)
...
!      call wall_clock(t2)
!      if(my_rank.eq.0)then
!        write(7,'(a,1pe14.5)')'density_gyro index',t2-t1
!      endif
!      call wall_clock(t1)

!$omp  target teams distribute parallel do collapse(3) thread_limit(256) &
!$omp& private(n,nc,ia,ja,ka,m,p0,p1,p2,mu1)
!$omp& private(s1,s2,s3,s4,s5,s6,s7,s8,u1,u2,u3,u4,u5,u6,u7,u8)
!$omp& private(v1,v2,v3,v4,v5,v6,v7,v8,w1,w2,w3,w4,w5,w6,w7,w8)
      do ka = 1, lphi-1
      do ja = 1, lz-1
...
! !$omp  simd reduction(+:s1,s2,s3,s4,s5,s6,s7,s8,u1,u2,u3,u4,u5,u6,u7,u8) &
! !$omp& simd reduction(+:v1,v2,v3,v4,v5,v6,v7,v8,w1,w2,w3,w4,w5,w6,w7,w8)
      do m = 1, imrk_grid(ia,ja,ka)
...
!      call wall_clock(t2)
!      if(my_rank.eq.0)then
!        write(7,'(a,1pe14.5)')'density_gyro grid sum',t2-t1
!      endif
!      call wall_clock(t1)
      
!$omp  target teams distribute parallel do
!$omp& private(m,n,nc,ia,ja,ka)
!$omp& private(p0,p1,p2,mu1)
       do m = 1, imrk_dead
        n =  marker_index_dead(m)
        nc =(n-1)/ngyro + 1
...
!       call wall_clock(t2)
!       if(my_rank.eq.0)then
!         write(7,'(a,1pe14.5)')'density_gyro dead sum',t2-t1
!       endif
!       call wall_clock(t1)
...
!       call wall_clock(t2)
!       if(my_rank.eq.0)then
!         write(7,'(a,1pe14.5)')'density_gyro work sum',t2-t1
!       endif
!       call wall_clock(t1)
...
!       call wall_clock(t2)
!       if(my_rank.eq.0)then
!         write(7,'(a,1pe14.5)')'density_gyro particle_mlt4b',t2-t1
!       endif
!       call wall_clock(t1)
...
!       call wall_clock(t2)
!       if(my_rank.eq.0)then
!         write(7,'(a,1pe14.5)')'density_gyro partsm1wopf',t2-t1
!       endif
!       call wall_clock(t1)
...
!       call wall_clock(t2)
!       if(my_rank.eq.0)then
!          write(7,'(a,1pe14.5)')'density_gyro field_mlt4',t2-t1
!       endif
!       call wall_clock(t1)
...
!       call wall_clock(t2)
!       if(my_rank.eq.0)then
!          write(7,'(a,1pe14.5)')'density_gyro last part',t2-t1
!       endif
...
!$omp  target teams distribute parallel do collapse(3) thread_limit(256) &
!$omp& private(n,nc,ia,ja,ka,m,p0,p1,p2,mu1,p3,p1mu)
!$omp& private(s1,s2,s3,s4,s5,s6,s7,s8,u1,u2,u3,u4,u5,u6,u7,u8)
!$omp& private(v1,v2,v3,v4,v5,v6,v7,v8,w1,w2,w3,w4,w5,w6,w7,w8)
!$omp& private(x1,x2,x3,x4,x5,x6,x7,x8,y1,y2,y3,y4,y5,y6,y7,y8)
      do ka = 1, lphi-1
